...
package body Humanize.Datetimes.Support is
   ...
   begin
      ...
      return Natural_Text (Natural (Year)) & "-"
        & Padded_Text (Natural (Month), 2) & "-"
        & Padded_Text (Natural (Day), 2);
   end ISO_Date;
      ...
      if Options.Calendar_Words_Only then
         ...
         declare
            Slavic : constant Humanize.Status.Text_Result :=
              Humanize.Datetimes.Support.Relative_Locales.Slavic_Relative_Result
                (Context, Selection);
         begin
         ...
         declare
            Generated_Future : constant Humanize.Status.Text_Result :=
              Humanize.Datetimes.Support.Relative_Locales.Generated_Future_Relative_Result
                (Context, Selection);
         begin
   ...
   begin
      return Natural_Text (Natural (Item.Year))
   ...
   begin
      return Natural_Text (Value) & Ordinal_Suffix (Value);
   end Ordinal_Day;
   ...
   is
      Base : constant String :=
        (if Options.Use_Month_Names then
            ...
            & (if Include_Year
               then ", " & Natural_Text (Natural (Item.Year))
               else "")
   ...
   is
      Year_Image : constant String :=
        Natural_Text (Natural (Item.Year));
   ...
   begin
      case Options.Style is
         ...
         when Calendar_Date_Quarter =>
            return "Q" & Natural_Text (Quarter) & " " & Year_Image;
         when Calendar_Date_Fiscal_Quarter =>
            return "FY" & Natural_Text (Fiscal_Year)
              & " Q" & Natural_Text (Fiscal_Quarter);
         ...
         when Calendar_Date_Fiscal_Half =>
            return "FY" & Natural_Text (Fiscal_Year)
              & " H" & Natural_Text (Fiscal_Half);
         when Calendar_Date_Semester =>
            return "S" & Natural_Text (Calendar_Half)
              & " " & Year_Image;
         when Calendar_Date_Half_Year =>
            return "H" & Natural_Text (Calendar_Half)
              & " " & Year_Image;
   ...
   begin
      if Item.Minute = 0 then
         return Natural_Text (Hour_12) & Suffix;
      else
         return Natural_Text (Hour_12) & ":"
         ...
         then
            return Ok_Text
              ((if Options.Include_Weekday then Weekday_Name (First) & " "
               ...
               & Natural_Text (Natural (First.Day)) & Sep
               & (if Options.Include_Weekday then Weekday_Name (Last) & " "
                  else "")
               & Natural_Text (Natural (Last.Day)));
            ...
            return Ok_Text
              (Date_Label (Context, First, Options, False) & Sep
               ...
               & ", " & Natural_Text (Natural (First.Year)));
         else
            return Ok_Text
   ...
   begin
      if Fiscal_Start = 1 then
         return "Q" & Natural_Text (Quarter) & " "
           & Natural_Text (Natural (Item.Year));
      else
         return "FY" & Natural_Text (Quarter_Year)
           & " Q" & Natural_Text (Quarter);
      end if;
   end Quarter_Label;
